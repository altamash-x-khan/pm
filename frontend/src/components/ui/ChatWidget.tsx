...
export function ChatWidget() {
  ...
  const startMode = useCallback((mode: Mode) => {
    setActiveMode(mode);
    let initialMessage = "";
  ...
  useEffect(() => {
    const handleOpenChat = (e: Event) => {
      ...
      setIsOpen(true);
      if (customEvent.detail?.mode && customEvent.detail.mode !== activeMode) {
        startMode(customEvent.detail.mode);
      }
    };
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="bg-gradient-to-r from-brand-rose to-[#c08b8b] text-white p-4 flex justify-between items-center shadow-sm shrink-0">
              <div>
                <h3 className="font-serif font-bold text-2xl leading-none text-white">Zara</h3>
                <p className="text-sm font-medium text-white/90 mt-1">Dr. Farheen's AI Assistant</p>
